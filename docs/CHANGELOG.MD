...
### Added
- [Pull 48](https://github.com/target111/merlin/pull/48) - Added `ls` command to agent & agent menu. Uses go code native to Merlin instead of executing `ls` or `dir` binary
- New `NativeCmd` message struct for commands native to Merlin
- [Pull 49](https://github.com/target111/merlin/pull/49) - Added tests for agent package and new `/test` directory for test HTTP server
- [Pull 50](https://github.com/target111/merlin/pull/50) - Adds a killdate option to agents
  - Added `set killdate` to Merlin Server agent menu and information table so that it can be changed from the server.
- [Pull 51](https://github.com/target111/merlin/pull/51) - Adds a Go native `minidump` module for Windows agents to create and receive a minidump file of a specified process (requires elevation)
...
- [Pull 58](https://github.com/target111/merlin/pull/58) - Added feature to generate in-memory TLS certificates one is not provided
...
- [Pull 60](https://github.com/target111/merlin/pull/60) - Added `pwd` and `cd` command to agent & agent menu. 
...
- [Pull 64](https://github.com/target111/merlin/pull/64) - Added in Dockerfile by Alex Flores (@audibleblink)

### Fixed
- [Pull 57](https://github.com/target111/merlin/pull/57) - Resolved broken JSON does not increment failedCheckin counter on Merlin agent
...
### Removed
- Removed publicly distributed certificates from repository see [Pull 58](https://github.com/target111/merlin/pull/58).
...
### Added
- [Pull 45](https://github.com/target111/merlin/pull/45) - Added a module for Python-based SOCKS proxying (@klusitc)
...
- [Pull 43](https://github.com/target111/merlin/pull/43) - Gives users the ability to dynamically
assign the callback URL variable at compile time by setting the URL= var in the make command by Alex Flores (@audibleblink)

### Fixed
- [Pull 44](https://github.com/target111/merlin/pull/44) Fixes case-sensitive autocompletion of `agent`
  on the module menu (@audibleblink)

## 0.6.5 - 2019-01-10

### Fixed
- [Issue 41](https://github.com/target111/merlin/issues/41) Error when downloading a file using a path containing a space
...
### Changed
- [Issue 26](https://github.com/target111/merlin/issues/26) Updated Makefile to remove .dmg file extension for Darwin
- [Issue 23](https://github.com/target111/merlin/issues/23) Error when using upload command without specific arguments
- [Issue 28](https://github.com/target111/merlin/issues/28) Updated MimiPenguin module to use new path to download mimipenguin.sh
...
### Fixed
- [Issue 23](https://github.com/target111/merlin/issues/23) Error writing an agent log entry when agent was set to 'all'
